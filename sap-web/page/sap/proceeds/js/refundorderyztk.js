...
function linkData(Src){
      ...
      function getBusinessData(data) {
            ...
            $.getJSON("../json/dataList.json",function(dataJson){
                  var dataList = dataJson["AR108"],//提取指定表单数据
                  //抬头表
                  len = Math.ceil(dataList.val.length / 2),str='';
                  var headLen = dataList.val.length;				                                                           
                  //表1详情
                  var t1Length = dataList.refundOrderItem.length;
                  var t1Head = '<tr><th>序号</th>';
                  var t1Body = '';
                  t1Str = '';
                  //表格一数据组数
                  var t1Len = data.refundOrderItem.length;
                  if(width > 520) {
                        //循环抬头数据
                        for(var i = 0 ; i < len ; i++){
                        ...
                        for(var i = 0;i < t1Length;i++) {
                              t1Head += "<th>" +dataList.refundOrderItem.Key[i] + "</th>";
                        }
                        for(var i = 0;i < t1Len;i++) {
                              t1Body += "<tr><td class='white'>"+(i+1)+"</td>";
                              for(var j = 0;j < t1Length;j++) {
                                    var key =  dataList.refundOrderItem.val[j];
                                    t1Body += "<td>" +(data.refundOrderItem[i][key] != null || '' ? data.refundOrderItem[i][key] : "空值") + "</td>";
                              }
                              t1Body += "</tr>";
                        } 
                        t1Head += "</tr>";  
                        ...
                        for(var i = 0;i < t1Len;i++) {
                              ...
                              for(var j = 0;j < t1Length;j++) {
                                    var key =  dataList.refundOrderItem.val[j];
                                    t1Str += "<tr><th>" + dataList.refundOrderItem.Key[j] + "</th><td>" + (data.refundOrderItem[i][key] != null || '' ? data.refundOrderItem[i][key]: '空值') + "</td></tr>";
                              }
                              t1Str += "</table>"
                        }
